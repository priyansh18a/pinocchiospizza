...
def model_dict(model, cat=None):
    ...
    # Remove extras from subs
        ...
        for k in keys:
            new[k] = model.objects.filter(category=cat).exclude(extra=True).values_list(k, flat=True).distinct()
    else:
        for k in keys:
            new[k] = model.objects.filter(category=cat).values_list(k,flat=True).distinct()
